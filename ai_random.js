...
function ai_random(game) {
    ...
    for ( var i=1; i<game.AREA_MAX; i++ ) {
        ...
        for ( var j=1; j<game.AREA_MAX; j++ ) {
            ...
            // if the defending area has the same number of dice, skip unless we're both at 8
            if ( defending_area.dice == game.adat[i].dice ) {
                if ( defending_area.dice < 8 ) continue;
            }
